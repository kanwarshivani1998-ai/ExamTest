...
export function Notes() {
  ...
  return (
    <div className="space-y-4 pb-4">
      <h1 className="text-lg font-bold text-white">{bi('Notes', 'नोट्स', lang)}</h1>

      <div>
        <h2 className="mb-2 text-sm font-semibold text-gray-200">{bi('Topic Notes', 'विषय नोट्स', lang)}</h2>
        {topics.length === 0 && <p className="text-xs text-gray-400">{bi('No topic notes yet.', 'अभी तक कोई विषय नोट नहीं।', lang)}</p>}
        {topics.map((t) => (
          <Card key={t.id} className="mb-2"><CardContent>
            ...
            <textarea
              ...
              className="mt-2 w-full rounded-lg border border-white/10 p-2 text-xs"
              rows={2}
            />
          </CardContent></Card>
        ))}
      ...
      <div>
        <h2 className="mb-2 text-sm font-semibold text-gray-200">{bi('Question Notes', 'प्रश्न नोट्स', lang)}</h2>
        {stats.length === 0 && <p className="text-xs text-gray-400">{bi('No question notes yet.', 'अभी तक कोई प्रश्न नोट नहीं।', lang)}</p>}
        {stats.map((s) => {

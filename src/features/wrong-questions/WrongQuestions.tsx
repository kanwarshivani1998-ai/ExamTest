  ...
  return (
    <div className="space-y-3 pb-4">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-bold text-white">{bi('Wrong Questions', 'गलत प्रश्न', lang)}</h1>
        <Button size="sm" onClick={() => navigate('/practice?mode=wrong')}>{bi('Practice these', 'इनका अभ्यास करें', lang)}</Button>
      </div>
      {list.length === 0 && <p className="text-sm text-gray-400">{bi('No wrong attempts yet — good job!', 'अभी तक कोई गलत उत्तर नहीं — बढ़िया!', lang)}</p>}
      {list.map((q) => {
